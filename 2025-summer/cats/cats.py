...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    satisfied_paragraphs = list(filter(select, paragraphs))
    if len(satisfied_paragraphs) <= k:
        return ''
    return satisfied_paragraphs[k]
    # END PROBLEM 1
...
def about(keywords):
    ...
    def aux(paragraph):
        ...
        normalized_paragraph = split(lower(remove_punctuation(paragraph)))
        #print("DEBUG: ", normalized_paragraph)
        for word in normalized_paragraph:
            print("DEBUG: ", word)
            if word in keywords:
                return True
        return False
