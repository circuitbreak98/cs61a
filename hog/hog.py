...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    a,b = opponent_score//10%10, player_score%10
    sub = a-b
    if sub<0:
        sub = -sub
    elif sub==0:
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    score_factors_count = num_factors(score)
    if score_factors_count == 3 or score_factors_count == 4:
        n=score
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who==0:
            ...
            score0 = update(num_dices, score0, score1, dice)
        else:
            num_dices = strategy1(score1, score0)
            score1 = update(num_dices, score1, score0, dice)
        who = 1 - who
    # END PROBLEM 5
...
def is_always_roll(strategy, goal=GOAL):
    ...
    is_always_same = True
    score0, score1 = 0, 0
    roll_num = strategy(score0, score1)
    while score0 < goal:
        while score1 < goal:
            if roll_num != strategy(score0, score1):
                is_always_same = False
            score1 +=1
        score0 +=1
        score1 = 0
    return is_always_same
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, max_scoring_num = 0, 0
    num_rolls_max = 1
    while num_rolls < 10:
        num_rolls+=1
        curr = make_averaged(roll_dice, times_called)(num_rolls, dice)
        if max_scoring_num < curr:
            max_scoring_num = curr
            num_rolls_max = num_rolls
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    boar_score = boar_brawl(score, opponent_score)
    sus_score = sus_update(0, score, opponent_score, six_sided)
